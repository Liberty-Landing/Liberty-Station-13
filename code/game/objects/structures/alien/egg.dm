	...
	if(progress >= MAX_PROGRESS)
		for(var/mob/M in GLOB.dead_mob_list)
			if(isghost(M) && M.client && M.client.prefs && (ROLE_XENOMORPH in M.client.prefs.be_special_role))
				to_chat(M, SPAN_NOTICE("An scourgy is ready to hatch! ([ghost_follow_link(src, M)]) (<a href='byond://?src=\ref[src];spawn=1'>spawn</a>)"))
		STOP_PROCESSING(SSobj, src)
		update_icon()
...
/obj/structure/alien/egg/attack_ghost(var/mob/observer/ghost/user)
	...
	if(jobban_isbanned(user, "Xenomorph"))
		to_chat(user, SPAN_DANGER("You are banned from playing a Scourge."))
		return

	var/confirm = alert(user, "Are you sure you want to join as a Scourgeling?", "Become Scourgeling", "No", "Yes")
