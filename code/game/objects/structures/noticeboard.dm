...
/obj/structure/noticeboard/marshal
	name = "Watchmen bulletin board"
	desc = "A board containing vital notices and official memos for Watchmen personnel."
	icon_state = "nboard03"
	notices = 3
...
<small>{It's not a shameful thing to look back on your knowledge for help, some days we all need a hand. Being able to keep an active mind like aluminium and the ability to accurately divine the law will be a well  used skill. Much like our local church they teach us to keep body spirit and mind in balance, so to must we learn to improve on these with mental drills, situational training with our team, and rest when we need it.}</small></ul> <hr><hr>"
	P.copy_overlays(list("paper_stamp-dots"), TRUE)
	P.stamped &= STAMP_FACTION
	src.contents += P
...
/obj/structure/noticeboard/research/New()
	...
	<li> Not turn off the Sonic Fence or allow it to be turned off. The Sonic Fence is a perfect containment to nacre, when it is active. If it turned off, the crystal will be able to spread out of control. If it needs to be turned off for whatever reason, either keep an eye on the crystals, or harvest them all to prevent unwanted spread.\
	<li> Reseed the field. In case the field get removed completely, more crystals can be seeded with either 5 nacre shard, or by splashing at least 10u of Liquid nacre on the floor.\
	...
	<li> Carry the shards through the fence. Your body interfere with the Sonic Fence, allowing you to carry nacre shards through the fence. The same cannot be said for dragged or thrown crystals, which will get destroyed on contact. Do not throw money away.</ol> "
	P.copy_overlays(list("paper_stamp-dots"), TRUE)
	P.stamped &= STAMP_FACTION
	src.contents += P
