/obj/item/camera_assembly
	...
	matter = list(MATERIAL_ALUMINIUM = 2, MATERIAL_GLASS = 3)

	//	Motion, EMP-Proof, X-Ray
	var/list/obj/item/possible_upgrades = list(/obj/item/device/assembly/prox_sensor, /obj/item/stack/material/composite, /obj/item/stock_parts/scanning_module)
...
/obj/item/camera_assembly/attackby(obj/item/I, mob/living/user)
	...
	switch(tool_type)
		...
		if(QUALITY_SCREW_DRIVING)
			if(state == 3)
				if(I.use_tool(user, src, WORKTIME_FAST, tool_type, FAILCHANCE_VERY_EASY, required_stat = STAT_MEC))
					var/input = sanitize(input(usr, "Which networks would you like to connect this camera to? Separate networks with a comma. No Spaces!\nFor example: Liberty colony,Security,Secret", "Set Network", camera_network ? camera_network : NETWORK_CEV_ERIS))
					if(!input)
