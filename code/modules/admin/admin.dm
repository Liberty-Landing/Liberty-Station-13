	...
	switch(admincaster_screen)
		...
		if(9)
			...
			if(src.admincaster_feed_channel.censored)
				dat+={"
					<FONT COLOR='red'><B>ATTENTION: </B></FONT>This channel has been deemed as threatening to the welfare of the Liberty colony, and marked with a [company_name] D-Notice.<BR>
...
/datum/admins/proc/output_ai_laws()
	...
	for(var/mob/living/silicon/S in SSmobs.mob_list)
		...
		if(isAI(S))
			...
			to_chat(usr, "<b>CYBORG [key_name(S, usr)] [R.connected_ai?"(Sovereigned to: [R.connected_ai])":"(Independant)"]: laws:</b>")
		else if (ispAI(S))
