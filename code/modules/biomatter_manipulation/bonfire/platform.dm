...
/obj/machinery/multistructure/bonfire_part/platform
	...
	desc = "The platforms of the Bonfire, that get filled with solution to melt down anything within into scorch to be reshaped by the Custodians."
...
/obj/machinery/multistructure/bonfire_part/platform/Initialize(mapload)
	. = ..()
	update_icon()

/obj/machinery/multistructure/bonfire_part/platform/Process()
	if(!MS)
		use_power(1)
		return
	if((!is_breached() || MS_bioreactor.is_operational()) && MS_bioreactor.chamber_solution)
		...
		for(var/atom/movable/M in loc)
			...
			if(isliving(M))
				...
				var/damage = BONFIRE_DAMAGE_PER_TICK - (BONFIRE_DAMAGE_PER_TICK * hazard_protection)
				victim.apply_damage(damage, BURN, used_weapon = "Scorch Exposure")
				victim.adjustOxyLoss(BONFIRE_DAMAGE_PER_TICK / 2)	// Snowflake shit, but we need the mob to die within a reasonable time frame
...
/obj/machinery/multistructure/bonfire_part/platform/proc/consume(atom/movable/object)
	if(ishuman(object))
		...
		for(var/obj/item/item in H.contents)
			...
			if(istype(item, /obj/item/organ))
				...
				if(istype(organ, /obj/item/organ/external) && (organ.nature == MODIFICATION_ORGANIC || organ.nature == MODIFICATION_SUPERIOR))
					continue
				var/obj/machinery/multistructure/bonfire_part/platform/neighbor_platform = pick(MS_bioreactor.platforms)
...
/obj/machinery/multistructure/bonfire_part/platform/update_icon()
...
/obj/machinery/multistructure/bonfire_part/platform/proc/get_opened_dirs()
...
/obj/machinery/multistructure/bonfire_part/platform/proc/make_windows()
...
/obj/machinery/multistructure/bonfire_part/platform/proc/apply_window(obj/structure/window/reinforced/glass, var/direction)
...
/obj/machinery/multistructure/bonfire_part/platform/proc/is_breached()
...
/obj/structure/window/reinforced/bioreactor
	name = "bonfire glass"
	desc = "The reinforced glass walls of the Bonfire platforms, to allow Custodians to see its interior procedure safely."
	...
	if(istype(I, /obj/item/mop) || istype(I, /obj/item/soap))
		...
		if(do_after(user, CLEANING_TIME * contamination_level, src))
			...
			if(contamination_level >= 4)
				spill_scorch(user.loc, cardinal)
		else
