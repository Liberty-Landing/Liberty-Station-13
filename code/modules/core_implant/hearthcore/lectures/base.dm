...
/datum/lecture/hearthcore/base/entreaty
	name = "Entreaty"
	phrase = "Oxidate Lecture: Entreaty."
	desc = "Call for help, allowing other Hearthcore users to hear your cries."
	power = 10

/datum/lecture/hearthcore/base/entreaty/perform(mob/living/carbon/human/H, obj/item/implant/core_implant/C)
	for(var/mob/living/carbon/human/target in disciples) //calls all hearthcore users
		if(target == H)
			continue

		var/obj/item/implant/core_implant/hearthcore/CI = target.get_core_implant()
		var/area/t = get_area(H) //get the area name

		if((istype(CI) && CI.get_module(HEARTHCORE_COMMON)) || prob(50)) //if we found someone, 50/50 chance we print out a help message
			to_chat(target, SPAN_DANGER("[H], fellow Hearthcore bearer, cries for help at [t.name]!"))
	return TRUE

/datum/lecture/hearthcore/base/pyrelight
	name = "Pyrelight"
...
/datum/lecture/hearthcore/base/pyrelight/perform(mob/living/carbon/human/H, obj/item/implant/core_implant/C)
...
/datum/lecture/hearthcore/base/message
	name = "Carrier Pigeon"
	phrase = "Oxidate Lecture: Carrier Pigeon."
	desc = "Create a pigeon of light to send a message to a person of your choice."
	power = 10

/datum/lecture/hearthcore/base/message/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C,list/targets)
...
/datum/lecture/hearthcore/base/revelation
	name = "Revelation"
...
/datum/lecture/hearthcore/base/revelation/perform(mob/living/carbon/human/H, obj/item/implant/core_implant/C)
	var/mob/living/carbon/human/lectured = get_front_human_in_range(H, 4)
	if(!lectured)
		fail("No target.", H, C)
		return FALSE
	give_sanity_boost(lectured) //trigger boosting proc
	LEGACY_SEND_SIGNAL(H, COMSIG_LECTURE, src, lectured) //not relevant to the lecture itself, just used for a personal objective if it exists
	return TRUE

/datum/lecture/hearthcore/base/revelation/proc/give_sanity_boost(mob/living/carbon/human/lectured)
...
/datum/lecture/hearthcore/base/revelation/proc/take_sanity_boost(mob/living/carbon/human/lectured)
	if(!lectured)
		return
...
/datum/lecture/hearthcore/base/empower
	name = "Empower"
...
/datum/lecture/hearthcore/base/empower/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C,list/targets)
...
/datum/lecture/hearthcore/base/accelerated_growth
...
/datum/lecture/hearthcore/base/accelerated_growth/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C)
...
/datum/lecture/hearthcore/base/accelerated_growth/proc/give_boost(datum/seed/S)
...
/datum/lecture/hearthcore/base/accelerated_growth/proc/take_boost(datum/seed/S)
...
/datum/lecture/hearthcore/base/mercy
	name = "Hand of Mercy"
...
/datum/lecture/hearthcore/base/mercy/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C)
...
/datum/lecture/hearthcore/base/absolution
...
/datum/lecture/hearthcore/base/absolution/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C,list/targets)
	var/mob/living/carbon/human/T = get_front_human_in_range(user, 1)
	if(!T) //anyone there?
		fail("No target in front of you.", user, C)
		return FALSE
...
/datum/lecture/hearthcore/base/purging
...
/datum/lecture/hearthcore/base/purging/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C)
	var/mob/living/carbon/human/T = get_front_human_in_range(user, 1)
	if(!T)
		fail("No target in front of you.", user, C)
		return FALSE
...
/datum/lecture/hearthcore/base/records
...
/datum/lecture/hearthcore/base/records/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C)
...
/datum/lecture/hearthcore/base/anti_scrying
	name = "Anti-Scrying"
...
/datum/lecture/hearthcore/base/anti_scrying/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C)
	if(C.get_module(HEARTHCORE_ANTI_SCRYING)) //if you already have the ANTI_SCRYING module, remove it
		C.remove_modules(HEARTHCORE_ANTI_SCRYING)
		to_chat(user, SPAN_NOTICE("You restore the Radiance behind your eyes, allowing Scrying attempts."))
		return TRUE
	else
		C.add_module(new HEARTHCORE_ANTI_SCRYING) //if there's no ANTI_SCRYING module, add it
...
/datum/lecture/hearthcore/priest/acolyte/search/perform(mob/living/carbon/human/user, obj/item/implant/core_implant/C, list/targets)
	var/list/OBJS = get_front(user)
...
//datum/lecture/hearthcore/base/wardenpyre LATER

//datum/lecture/hearthcore/base/cauterization MAYBE LATER
