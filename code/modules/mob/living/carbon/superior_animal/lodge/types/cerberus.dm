...
/mob/living/carbon/superior_animal/lodge/cerberus
	name = "cerberus"
	desc = "A sturdy, intelligent, evolved strain of Hell Diver generally domesticated by Fontaine trappers to be used as hunting companions or pets. Favored especially for their asexual \
	reproduction through a mushroom based diet, a fact that still baffles some Phokorus Institute scientists. Unlike a standard Hell Diver, the Cerberus isn't as \
	...
	if(istype(O, /obj/item/reagent_containers/food/snacks/grown)) //feedin' dem chickens
		...
		if(G.seed && G.seed.kitchen_tag == "mushroom")
			if(!stat && offspring_left < 1)
				user.visible_message("\blue [user] feeds [O] to [name]! It snuffles happily.","\blue You feed [O] to [name]! It snuffles happily.")
				user.drop_item()
				qdel(O)
				offspring_left += 1
			else
