	...
	for(var/atom/movable/stomachContent in contents)
		if(prob(digestionProbability))
			if(istype(stomachContent,/obj/item/stack)) //converts to plasma, keeping the stack value
				if(!istype(stomachContent,/obj/item/stack/material/hydrogenc))
					var/obj/item/stack/oldStack = stomachContent
					new /obj/item/stack/material/hydrogenc(src, oldStack.get_amount())
					qdel(oldStack)
					continue
			else if(istype(stomachContent,/obj/item)) //converts to plasma, keeping the w_class
				var/obj/item/oldItem = stomachContent
				new /obj/item/stack/material/hydrogenc(src, oldItem.w_class)
				qdel(oldItem)
				continue
			else
				new /obj/item/stack/material/hydrogenc(src, flatPlasmaValue) //just flat amount
				qdel(stomachContent)
				continue
